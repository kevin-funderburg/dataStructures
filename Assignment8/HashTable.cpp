...
void HashTable::rehash()
{
    ...
    while (i < used)
    {
        if (data[nxt].word[0] != '\0')
        {
            strcpy(temp[i].word, data[nxt].word);
            i++;
        }
        nxt++;
    }
...
bool HashTable::search(const char* cStr) const
{
    ...
    bool wordFound  = (strcmp(data[loc_0].word, cStr) == false);
...
{
    size_type location = hash(cStr) % capacity,
              location_n = location,
              i = 0;

    // Quadratic probing method
    while ( data[location_n].word[0] != '\0' )
    {
        location_n = (location + i * i) % capacity;
        i++;
    }

    strcpy(data[location_n].word, cStr);
    used++;
