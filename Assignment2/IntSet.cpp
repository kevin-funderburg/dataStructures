...
IntSet::IntSet(int initial_capacity) : capacity(DEFAULT_CAPACITY), used(0)
{
...
bool IntSet::isSubsetOf(const IntSet& otherIntSet) const
{
	...
	for (int i = 0; i < used; i++)
		if (otherIntSet.contains(data[i]) == false) return false;
	return true;
}
...
{
	IntSet newSet = *this;
	if (used + otherIntSet.used > capacity)
		newSet.resize( int(1.5 * (used + otherIntSet.used) + 1) );
...
IntSet IntSet::intersect(const IntSet& otherIntSet) const
{
	...
	for (int i = 0; i < newSet.used; i++) {
		if (otherIntSet.contains(newSet.data[i]) == false)
		{
			newSet.remove(newSet.data[i]);
			i -= 1;
		}
	}
	return newSet;
...
IntSet IntSet::subtract(const IntSet& otherIntSet) const
{
	IntSet newSet = *this;
	for (int i = 0; i < otherIntSet.used; i++)
		newSet.remove(otherIntSet.data[i]);
	return newSet;
}
...
{
	if (contains(anInt) == true) return false;
	else {
		if (used == capacity) {
...
bool operator==(const IntSet& is1, const IntSet& is2)
{
	...
	if (is1.isEmpty() == true && is2.isEmpty() == true) return true;
	bool isEqual = is2.isSubsetOf(is1);
	return isEqual;
}
